(() => {
  ...
  async function getClient(name) {
    ...
    return gradio.Client.connect(name);
  }
  ...
  async function mergeAudio(blobs) {
    ...
    const ctx = new AC(); const decoded=[];
  ...
  async function generateVoxCPM(chunk, ref) {
    ...
    const refArg = mode==='clone' ? gradio.handle_file(ref) : null;
  ...
  async function generateOmni(chunk, ref) {
    ...
    const result = await client.predict('/_clone_fn', [chunk, 'Burmese', gradio.handle_file(ref), '', style.value.trim(), 32, 2.0, true, 1.0, 0, true, true]);
  ...
  async function withRetry(fn, attempts=RETRIES) {
    ...
    for (let n=1; n<=attempts; n++) {
      ...
      catch (e) { lastErr=e; if (n<attempts) { setStatus(`ပြန်ကြိုးစားနေသည်… (${n+1}/${attempts})`); await new Promise(r=>setTimeout(r, 1200*n)); } }
    }
  ...
  async function runLongText() {
    ...
    setStatus(`စာသား ${text.value.trim().length.toLocaleString()} characters → ${chunks.length} parts ခွဲပြီး စတင်နေသည်…`);
    let engines = selected==='auto' ? (mode==='clone' ? ['voxcpm','omnivoice'] : ['voxcpm']) : [selected];
    const blobs=[]; let engineUsed=null;
    for(let i=0;i<chunks.length;i++){
      let lastErr=null;
      for(const eng of engines){
        try{
          if(eng==='omnivoice' && mode!=='clone') throw Error('OmniVoice fallback သည် Voice Cloning mode မှာပဲ သုံးပါမယ်');
          setStatus(`${eng==='voxcpm'?'VoxCPM2':'OmniVoice'} — အပိုင်း ${i+1}/${chunks.length} ကို အသံဖန်တီးနေသည်…`);
          const audio=await withRetry(() => eng==='voxcpm' ? generateVoxCPM(chunks[i],fileData) : generateOmni(chunks[i],fileData));
          blobs.push(await toBlob(audio, eng==='voxcpm'?PROVIDERS.voxcpm:PROVIDERS.omnivoice)); engineUsed=engineUsed||eng; lastErr=null; break;
        }catch(e){lastErr=e; if(eng===engines[engines.length-1]) throw e; setStatus(`${eng} မအောင်မြင်ပါ — အခြား engine သို့ပြောင်းနေသည်…`);}
      }
      progress(((i+1)/chunks.length)*100, `အပိုင်း ${i+1}/${chunks.length} ပြီးပါပြီ`);
    }
    setStatus('အသံအပိုင်းများကို တစ်ဖိုင်တည်း ပြန်ပေါင်းနေသည်…');
    const finalBlob=await mergeAudio(blobs); return {blob:finalBlob, engine:engineUsed, chunks:chunks.length};
  }
  ...
  document.addEventListener('pointerdown', event=>{const ripple=document.createElement('span');ripple.className='touch-ripple';ripple.style.left=`${event.clientX}px`;ripple.style.top=`${event.clientY}px`;document.body.appendChild(ripple);ripple.addEventListener('animationend',()=>ripple.remove());});
